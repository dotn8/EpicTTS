    ...
    public class ShellContextMenu : NativeWindow
    {
        ...
        private IShellFolder GetParentFolder(string folderName)
        {
            if (null == _oParentFolder)
            {
                ...
                int nResult = oDesktopFolder.ParseDisplayName(IntPtr.Zero, IntPtr.Zero, folderName, ref pchEaten,
        ...
        protected IntPtr[] GetPIDLs(FileInfo[] arrFI)
        {
            ...
            foreach (var fi in arrFI)
            {
                ...
                SFGAO pdwAttributes = 0;
                IntPtr pPIDL = IntPtr.Zero;
                int nResult = oParentFolder.ParseDisplayName(IntPtr.Zero, IntPtr.Zero, fi.Name, ref pchEaten, out pPIDL,
                    ref pdwAttributes);
                if (S_OK != nResult)
                {
                    FreePIDLs(arrPIDLs);
        ...
        private void InvokeContextMenuDefault(FileInfo[] arrFI)
        {
            ...
            try
            {
                ...
                {
                    InvokeCommand(_oContextMenu, nDefaultCmd, arrFI[0].DirectoryName, new Point(Control.MousePosition.X, Control.MousePosition.Y));
                }
